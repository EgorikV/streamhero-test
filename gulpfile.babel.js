...
import WEBCLIENT_PATH from './scripts/webclient-path'
import webclientFile from './scripts/webclient-file'
...
gulp.task('build',
  gulp.series(clean, robots, pages, sass, images, inline));

// Build the "dist" folder by running all of the above tasks
gulp.task('webclient',
  gulp.series('build', webclient));
...
// Copy body HTML to webclient folder
function webclient() {
  console.log(`Destination path is "${WEBCLIENT_PATH}"`)

  return gulp.src('dist/**/*.html')
    .pipe(webclientFiles())
    .pipe(gulp.dest(WEBCLIENT_PATH));
}
...
function webclientFiles() {
  var pipe = lazypipe()
    .pipe($.filter, [
      'dist/webclient/*.html',
      '!dist/webclient/base.html',
      '!dist/webclient/example.html',
      '!dist/webclient/index.html',
      '!dist/webclient/receipt.html',
      '!dist/webclient/welcome.html',
    ])
    .pipe($.rename, {dirname: ''})
    .pipe($.replace, /(((.+\n)+)(.+)\n*)<!-- body -->(((\n*.+\n)+)(.+)\n*)<!-- body end -->(((\n*.+\n)+)(.+))/g, '$5')
    .pipe($.tap, file => file.contents = Buffer.from(webclientFile(file.path, file.contents.toString())));
